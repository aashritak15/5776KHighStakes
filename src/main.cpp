...
void fourRingBlue() {
    ...
    // // ram 1
    chassis.moveToPoint(29.9, 5.9, 1000, {.minSpeed = 50});
    ...
    chassis.waitUntilDone();
    intakeState = 1;
    // chassis.moveToPoint(29.9, 5.9, 1000, {.minSpeed = 30});
    // chassis.waitUntilDone(); // go to ring stack
    // pros::delay(100);
    // intakeState = 1;
    ...
    // chassis.waitUntilDone();
    // intakeState = 2;
    // chassis.moveToPoint(30.9, 6.9, 750, {.minSpeed = 50});
    // chassis.waitUntilDone(); // go to ring stack
    // pros::delay(100);
    // intakeState = 1;
...
void skills() {
    ...
    //back up and get mogo rings #3 4 5
    chassis.moveToPoint(26, 44.75, 500, {.forwards = false});
    chassis.waitUntilDone();
    ...
    intakeLower.move_velocity(-6000);
    ...
    //clamp to push
    chassis.turnToHeading(180, 500);
    chassis.moveToPoint(6, 91.9, 1000, {.forwards = false});
    chassis.waitUntilDone();
    intakeKeep = false;
    clampState = 1;
    pros::delay(500);
    ...
    //turn to corner
    chassis.turnToHeading(-105.8, 500);
    chassis.waitUntilDone();
    clampState = 0;

    chassis.moveToPoint(30.5, 99, 2250, {.forwards = false, .minSpeed = 100}); //push it into the corner 
    ...
    chassis.moveToPoint(-11.8, 26.9, 2000, {.forwards = false, .maxSpeed = 90});
    chassis.waitUntilDone();
    globalTarget = 0;
