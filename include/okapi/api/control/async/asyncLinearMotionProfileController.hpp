...
class AsyncLinearMotionProfileController : public AsyncPositionController<std::string, double> {
    public:
         ...
         */
        AsyncLinearMotionProfileController(const TimeUtil& itimeUtil, const PathfinderLimits& ilimits,
         ...
         * @param ibackwards Whether to follow the profile backwards.
         */
        void moveTo(const QLength& iposition, const QLength& itarget, const PathfinderLimits& ilimits,
    ...
    protected:
        ...
        std::string getPathErrorMessage(const std::vector<PathfinderPoint>& points, const std::string& ipathId,
                                        int length);
};
} // namespace okapi
